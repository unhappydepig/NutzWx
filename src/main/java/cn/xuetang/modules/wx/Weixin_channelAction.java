...
public class Weixin_channelAction extends BaseAction {
    ...
    @At("")
    @Ok("->:/private/wx/Weixin_channel.html")
    ...
    @At
    @Ok("->:/private/wx/Weixin_channelAdd.html")
    ...
    public boolean add(@Param("..") Weixin_channel wx, @Param("id") String id, HttpSession session) {
        ...
        if (!Strings.isBlank(wx.getId())) {
            Sql sql = Sqls
                    .create("select max(location)+1 from Weixin_channel where id like  @id");
    ...
    @At
    @Ok("->:/private/wx/Weixin_channelUpdate.html")
        ...
        try {
            for (int i = 0; i < id.length; i++) {
                daoCtl.exeUpdateBySql(
                        dao,
                        Sqls.create("delete from Weixin_channel where id like '"
                                + Strings.sNull(id[i]) + "%'"));
            }
        } catch (Exception e) {
            e.printStackTrace();
    ...
    private List getJSON(Dao dao, String id, int proid) {
        ...
        for (int i = 0; i < list.size(); i++) {
            ...
            if (i == 0 || "".equals(pid))
                pid = "0";
            int num = daoCtl.getRowCount(dao, Weixin_channel.class,
     ...
     */
    @At
    @Ok("->:/private/wx/Weixin_channelSort.html")
    ...
    public boolean sort(@Param("checkids") String[] checkids) {
        boolean rs = daoCtl.updateSortRow(dao, "Weixin_channel", checkids,
                "location", 0);
        return rs;
