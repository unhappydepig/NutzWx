...
public class LoginAction extends BaseAction {
	...
	@At("/doLogin")
	public String login(@Param("loginname") String loginname, @Param("password") String password, @Param("verifcode") String verifcode, HttpSession session, HttpServletRequest req) {
		...
		if (!Lang.digest("MD5", Strings.sNull(password).getBytes(), Strings.sNull(user.getSalt()).getBytes(), 3).equals(user.getPassword())) {
			int all = 5;
			int count = NumberUtils.toInt(Strings.sNull(session.getAttribute("errorlogincount")), 0);
			if (count > 4) {
				if (user.getState() == 0)
